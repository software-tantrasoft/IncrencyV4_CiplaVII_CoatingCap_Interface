...
class IncompleteGranulationData {
    async saveIncompleteData(cubicalObj,data,actualSampleValue, intNos, typeValue, tempUserObject, IdsNo) {
       ...
       var side = data.side
        ...
        if (masterTable == "tbl_tab_master17_incomplete" || masterTable == "tbl_cap_master17_incomplete") {
            if (actualSampleValue <= noOfSample) {
                const res = await proObj.productData(cubicalObj)

                var selectedType = globalData.arrPerFineTypeSelectedMenu.find(k => k.idsNo == IdsNo);
                // if(selectedType == "Compaction Granules"){

                // }

                // intNos = tempLimObj[selectedType.selectedPerFine].noOfSamples;
                // paramT1Pos = tempLimObj[selectedType.selectedPerFine].T1Pos;
                // paramT1Neg = tempLimObj[selectedType.selectedPerFine].T1Neg;
                if (selectedType.selectedPerFine == 'PerFineComp') {
                    var paramT1Neg = `Param8_Low`;
                    var paramT1Pos = `Param8_Upp`;
                    var PerFineHeading = '1';          //%FINE LAYRES 1.compaction 2.granyulation NEED TO UPDATED TO DB
                } else {
                    var paramT1Neg = `Param11_Low`;
                    var paramT1Pos = `Param11_Upp`;
                    var PerFineHeading = '2';         //%FINE LAYRES 1.compaction 2.granyulation NEED TO UPDATED TO DB
                }
                ...
                if (actualSampleValue == 1) {
                    ...
                    const masterObj = {
                        ...
                        data: [
                            ...
                            { str_colName: 'RepoLabel20', value: PerFineHeading },
